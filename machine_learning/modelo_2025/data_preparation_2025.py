"""
Preparação de dados para o modelo de ML de total_kills UNDER/OVER - DADOS 2025.
...
CSV_PATH = BASE_DIR / "data_2025_transformed.csv"  # Arquivo CSV transformado
OUTPUT_DIR = BASE_DIR / "data"
OUTPUT_DIR.mkdir(exist_ok=True)


def load_data() -> pd.DataFrame:
    """Carrega os dados de 2025."""
    if not CSV_PATH.exists():
        # Tenta arquivo original
        alt_path = BASE_DIR / "data_2025.csv"
        if alt_path.exists():
            print(f"Arquivo transformado não encontrado. Executando transformação...")
            import subprocess
            import sys
            subprocess.run([sys.executable, "transformar_dados.py"])
            if not CSV_PATH.exists():
                raise FileNotFoundError(f"Erro ao transformar dados. Arquivo {CSV_PATH} não encontrado!")
        else:
            raise FileNotFoundError(
                f"Arquivo {CSV_PATH} não encontrado!\n"
                f"Por favor, baixe o arquivo CSV do Google Drive e execute transformar_dados.py primeiro.\n"
                f"Link: https://drive.google.com/file/d/1v6LRphp2kYciU4SXp0PCjEMuev1bDejc/view?usp=drive_link"
            )
    
    print(f"Carregando dados de 2025 de {CSV_PATH}...")
    df = pd.read_csv(CSV_PATH)
    print(f"Dados carregados: {len(df)} partidas")
    
    # Verifica colunas necessárias
    required_cols = ['league', 'total_kills', 'top_t1', 'jung_t1', 'mid_t1', 'adc_t1', 'sup_t1',
...
def calculate_league_stats(df: pd.DataFrame) -> Dict[str, Dict[str, float]]:
    ...
    for league, stats in sorted(league_stats.items()):
        print(f"  {league}: média={stats['mean']:.2f}, std={stats['std']:.2f}, "
              f"amostras={len(df[df['league'] == league])}")
...
def create_features(df: pd.DataFrame, league_stats: Dict[str, Dict[str, float]], 
    ...
    champion_cols = ['top_t1', 'jung_t1', 'mid_t1', 'adc_t1', 'sup_t1',
                     'top_t2', 'jung_t2', 'mid_t2', 'adc_t2', 'sup_t2']
...
def main():
    ...
    print("=" * 60)
    print("PREPARAÇÃO DE DADOS PARA MODELO UNDER/OVER TOTAL_KILLS - 2025")
